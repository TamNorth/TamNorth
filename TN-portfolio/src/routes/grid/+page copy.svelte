<script lang="ts">
	...
	function fitPolygon({ polygonSides = 4, quadGroup, vertices, radius }) {
		...
		function resolveToPolygon(targetAngles, cornerVertices, vertexAngles, origin, radius) {
			...
			const remainingTargetAngles = [...targetAngles]; // polygon-to-fit angles e.g. [0, pi/2, pi, 3pi/4]
			let lastTargetAngle = targetAngles[targetAngles.length - 1]; // tracks through target angles e.g. 0 => pi/2 => pi => 3pi/4

			const remainingCornerIndices = cornerVertices.map(([id, index]) => index); // vertexAngleIndices e.g. [0, 3, 5, 8]
			let lastCornerIndex =
				vertexAngles.length - remainingCornerIndices[remainingCornerIndices.length - 1]; // tracks through cornerIndices e.g. 0 => 3 => 5 => 8
			// console.log('start');

			return vertexAngles.reduce((acc, [id, _], i) => {
				// console.log(remainingCornerIndices);
				if (cornerVertices.find(([cornerId, _]) => id === cornerId)) {
					...
					// console.log({ i });
					// console.log({ lastCornerIndex });
					// console.log({
					// 	denominator: (remainingCornerIndices[0] ?? vertexAngles.length) - lastCornerIndex
					// });
					const ratio =
						(i - lastCornerIndex) /
						Math.abs((remainingCornerIndices[0] ?? vertexAngles.length) - lastCornerIndex); // working
					// console.log({ ratio });
					const relativeAngle =
						(remainingTargetAngles[0] ?? targetAngles[0] + TWO_PI) - lastTargetAngle; // working
		...
		function anglesToVertices(angles, origin, radius) {
			const { x, y } = origin;
			return angles.map((angle) => {
				const targetX = x + radius * Math.cos(angle);
				const targetY = y + radius * Math.sin(angle);
				return { vertices: { x: targetX, y: targetY } };
			});
		}
