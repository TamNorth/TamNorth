<script lang="ts">
	...
	import { vertices as originalVertices, edges, quads } from './data.js';
	...
	const canvasFn = ({ canvas, overlayCanvas, mousePosition, mouseClick, w, h, offset }) => {
		...
		$effect(() => {
			...
			const polygonSides = 6; // Math.floor(Math.random() * 3 + 4);
		...
		$effect(() => {
			console.log(getNearestVertex(mousePos, vertices, scale, origin));
		});
